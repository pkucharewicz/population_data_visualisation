...
random_col=df.columns[-1]
year_Poland=random_col.split("_")[-1]
df['abs_difference_Poland_'+year_Poland] = abs(df[year_Poland]-df[year_Poland][df[df['Country Name']=='Poland'].index[0]])
df = df.sort_values(by='abs_difference_Poland_'+year_Poland).iloc[:5]
...
def animate(i):
    ...
    ax.set_xlabel('5 countries closest in population size to Poland'+ ' in '+year_Poland,fontsize=20, labelpad=30)
